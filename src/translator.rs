use crate::structs::{DbModbusData, HasData, Mapping, ModbusSensorData, ToDatabase, ValueType};
use chrono::Utc;
use evalexpr::*;
use lookups::{HashLookup, LkupHashMap, Lookup}; 

impl ToDatabase for ModbusSensorData {
    fn to_db_object<Y: HasData>(sensor_data: Y) -> DbModbusData {
            ...
            if sensor_mapping_result.is_some() {
                ...
                match sensor_mapping.mapping_type {
                    ...
                    ValueType::Combined => {
                        ...
                        let precompiled =
                            build_operator_tree::<DefaultNumericTypes>(address).unwrap();
...
pub fn translate_to_db_object<Y: HasData>(sensor_data: Y) -> DbModbusData {
    //TODO: panicked als er geen mapping is.
    let mut rdr = csv::ReaderBuilder::new()
        .has_headers(false)
        .from_path("mapping.csv")
